...
## Analysis of Illumina MethylationEPIC v2.0 beadchip datasets

The `r BiocStyle::Biocpkg("missMethyl")` package has been updated to support 
...
prior to analysis with the `r BiocStyle::Biocpkg("missMethyl")` package using 
the `r BiocStyle::Biocpkg("DMRcate")` package. An example of this is shown in 
this vignette.
...
## Filtering replicate probes in Illumina MethylationEPIC v2.0 beadchip datasets

The MethylationEPIC v2.0 array contains replicate probes which map to the same 
CpG dinucleotides. We recommend filtering these replicate probes using the 
`r BiocStyle::Biocpkg("DMRcate")` package as shown below. Please see the 
[`r BiocStyle::Biocpkg("DMRcate")` EPICv2 vignette](https://bioconductor.org/packages/release/bioc/vignettes/DMRcate/inst/doc/EPICv2.pdf) for more details.

```{r filter-replicate-probes_Mval, eval=FALSE}
### This code is not run within this vignette
...
```{r filter-replicate-probes_M, eval=FALSE}
### This code is not run within this vignette
...
argument can be specified as either "450K", "EPIC" or "EPIC_V2". The default is 
"450K".
